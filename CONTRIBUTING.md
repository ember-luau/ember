...
`target/debug/embr` by hand or you will run stale code.
...
| `src/sys/` | The `~/.ember` layout, subprocesses, git |
...
`embr-{os}-{arch}[.exe]`, because `embr self update` relies on both. Tags with a
